"""W5_PROTECTED_CONFIRMATION Family 2, step_a (diagnostic replication) + primary endpoints (1)-(2).
Byte-identical reuse of runs/AUCTION02_ACTION_RELEVANCE/src/02_step1_diagnostics.py's methodology
(stats_lib_auction02.run_cell / dual_block_bootstrap_corr / dual_block_bootstrap_meandiff,
imported UNMODIFIED from the frozen module) on the 8-session confirmation action_substrate.
Per spec.yaml's confirmation_procedure step_a: tercile cuts are RECOMPUTED on the confirmation
pool's own marginal distribution (a like-for-like re-diagnosis) -- this is explicitly NOT the
frozen policy trigger (CUT_FAR_TICKS, used only in the step_c construction script), so recomputing
here is not a violation of "no parameter changes."
"""
import os, sys, json
import numpy as np
import pandas as pd

AUCTION02_SRC = r"D:\OneDrive - Washington University in St. Louis\TradingResearch\systematic_research\runs\AUCTION02_ACTION_RELEVANCE\src"
sys.path.insert(0, AUCTION02_SRC)
from stats_lib_auction02 import run_cell, dual_block_bootstrap_corr, dual_block_bootstrap_meandiff  # noqa: E402
...
print(f"[step1-confirm] analysis_ok universe: n={len(ok)}, sessions={ok['sess_tag'].nunique()}", flush=True)

# ---- chronological half-split (same convention as the discovery script; with only 6 sessions
# contributing analysis_ok rows this is a 3-vs-3 session split, reported honestly) ----
...
print(f"[step1-confirm] chronological split: H1={sess_order[:mid]} ({mid} sessions), "
      f"H2={sess_order[mid:]} ({len(sess_order)-mid} sessions)", flush=True)
...
for prod, cfg in PRODUCTS.items():
    ...
    print(f"\n[step1-confirm] === Product {prod}: n={len(sub)}, trades={n_trades}, sessions={n_sess} ===", flush=True)
    results["cascade"][prod] = {"n": len(sub), "n_trades": n_trades, "n_sessions": n_sess}

    if len(sub) < 10:
        print(f"[step1-confirm] Product {prod}: n={len(sub)} too small for tercile cuts / cells, skipping", flush=True)
        results["per_product"][prod] = []
        results["cut_points"][prod] = None
        continue
    ...
    print(f"[step1-confirm] {prod} cut points (recomputed on confirmation pool, like-for-like "
          f"re-diagnosis, NOT the frozen CUT_FAR_TICKS policy trigger): "
          f"|value_dist_ticks| tercile={vd_cuts}, poc_share tercile={ps_cuts}", flush=True)
    ...
    print(f"[step1-confirm] {prod} confound check: {conf}", flush=True)
    ...
    # ---- PRIMARY ENDPOINT (1): dual-clustered bootstrap corr, value_dist_ticks_abs (+poc_share)
    # x signed_markout, every horizon ----
    boot_rows = []
    for pred_col, pred_label in [("abs_value_dist_ticks", "value_dist_ticks_abs"), ("poc_share", "poc_share")]:
        for H in HORIZONS:
    ...
    # ---- PRIMARY ENDPOINT (2): large-move probabilities, H=3 ----
    H = 3
    ...
    print(f"[step1-confirm] {prod} concentration check: {results['concentration_check'][prod]}", flush=True)

with open(os.path.join(OUT, "step1_results_CONFIRM.json"), "w") as f:
    json.dump(results, f, indent=2, default=float)
print("\nSTEP1_DIAGNOSTICS_CONFIRM DONE")
