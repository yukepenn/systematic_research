"""SMV2W_5MCLOCK_R2 -- Gate B (chronology): LOYO dSharpe same sign >= 4/5 years AND
fit 2022-24 -> eval 2025-26 point-positive, on the DUAL-transformed dev legs from
step1_dual_htf.py (out/curves.csv: DUAL_ALL, DUAL_5M).

Template reused verbatim from runs/SMV2T_NOFAST_R2/gate_B.py -- only the challenger
object (DUAL_5M in place of DUAL_NOFAST) changes; no redesign.
"""
import os, json
...
for yr, g in curves.groupby("year"):
    shp_all = sharpe(g["DUAL_ALL"]); shp_5m = sharpe(g["DUAL_5M"])
    d_sharpe = shp_5m - shp_all
    net_all = g["DUAL_ALL"].sum(); net_5m = g["DUAL_5M"].sum()
    loyo_rows.append({
        "year": int(yr), "n_days": len(g),
        "net_ALL": float(net_all), "net_5M": float(net_5m), "d_net": float(net_5m - net_all),
        "sharpe_ALL": shp_all, "sharpe_5M": shp_5m, "d_sharpe": d_sharpe,
...
def window_stats(g, label):
    ...
    return {
        ...
        "net_ALL": float(g["DUAL_ALL"].sum()), "net_5M": float(g["DUAL_5M"].sum()),
        "d_net": float(g["DUAL_5M"].sum() - g["DUAL_ALL"].sum()),
        "sharpe_ALL": shp_all, "sharpe_5M": shp_5m, "d_sharpe": shp_5m - shp_all,
    }
