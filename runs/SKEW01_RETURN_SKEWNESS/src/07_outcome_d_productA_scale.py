"""SKEW01 outcome (d) -- Product-A scale value: forward-20-bar bar_pnl_A_dollars summed and
divided by |target_exposure_A delta| (contracts_changed), U6's/U8's own exact
fwd20_pnl_per_contract construction, evaluated ONLY at action_A=='SCALE_IN' bars, feature =
aligned_skewness_20/10 (aligned to the scale-in's own side), primary; raw also reported.
...
COLS = ["t_idx", "sess_date", "year", "is_health_only_bar", "M_A_raw", "sigma460_atr_proxy_pts",
        "target_exposure_A", "action_A", "bar_pnl_A_dollars", "skewness_10", "skewness_20"]
...
scalein_side = np.sign(pos[idx])  # scale-in's own (post-scale) side, matches U6's own convention
for W in [10, 20]:
    ev[f"aligned_skewness_{W}"] = ev[f"skewness_{W}"] * scalein_side
