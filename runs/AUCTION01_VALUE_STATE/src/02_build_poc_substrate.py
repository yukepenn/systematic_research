"""AUCTION01 -- causal running POC/VWAP from raw trade prints, downsampled to
1s grid, merged onto grid1s + sechilo + U0 causal state. Writes one decision-
point table (post RTH+liquidity filter, 30s sampling) covering all 37
BBO-confirmed sessions.
"""
import os, glob
...
TICK = 0.25
EXCLUDE = {"20250811", "20250924", "20260430"}

sechilo_files = sorted(glob.glob(os.path.join(SECHILO, "s*.parquet")))
sessions = []
for f in sechilo_files:
    tag = os.path.basename(f).replace(".parquet", "").replace("s", "")
    assert tag not in EXCLUDE
    sessions.append(tag)
print(f"[build] {len(sessions)} BBO-confirmed sessions: {sessions[0]} .. {sessions[-1]}", flush=True)
...
for tag in sessions:
    ...
    if len(u0_sess) == 0:
        print(f"[build] {tag}: no U0 rows for sess_date {sd}, skipping session", flush=True)
        continue
    ...
    m["sess_tag"] = tag
    rows_all.append(m)
    print(f"[build] {tag}: {len(m)} 1s rows, poc range [{m['poc_price'].min():.2f},{m['poc_price'].max():.2f}], "
          f"U0 match rate {(m['M'].notna().mean()):.3f}", flush=True)

full = pd.concat(rows_all, ignore_index=True)
full.to_parquet(os.path.join(OUT, "poc_1s_full.parquet"), compression="zstd", index=False)
print(f"[build] full 1s table: {len(full)} rows across {full['sess_tag'].nunique()} sessions", flush=True)
...
dec.to_parquet(os.path.join(OUT, "decision_points_30s.parquet"), compression="zstd", index=False)
print(f"[build] decision points (RTH, liquid, 30s-sampled): {len(dec)} rows, "
