"""U8 outcome (c) -- reversal hazard: binary label = 1 if, within bars t+1..t+10 after a HOLD bar
t with position_B[t]=s (nonzero), EITHER action_B=='REVERSAL' occurs OR sign(M)=-s at any bar in
that window (M crosses against the held position); else 0. Same HOLD-bar population as outcome
(b). Forward-only by construction (window starts at t+1), no sunk-P&L confound."""
import os
import sys
import json
import numpy as np
...
COLS = ["t_idx", "sess_date", "year", "is_health_only_bar", "M", "sigma460_atr_proxy_pts",
        "position_B", "action_B", "sigma460_atr_proxy_pts"] + FEATURES_10 + FEATURES_20

df = pd.read_parquet(os.path.join(OUT, "u8_bars_with_features.parquet"), columns=list(dict.fromkeys(COLS)))
