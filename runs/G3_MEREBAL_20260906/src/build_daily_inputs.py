"""G3_MEREBAL_20260906 -- STEP 0: build ES + ZB causal daily inputs.

Same construction as G3_FTQGATE's data line ("ES + ZB causal daily"), built INDEPENDENTLY here
the certified way: research/multi_market/src/ncd_day.py (NT8 day .ncd reader, per-contract TRUE
data) + research/multi_market/src/roll.py (causal volume-crossover roll, self-financing
point return), identity-gated against roll.economic_returns, roll-causality asserted, seal
asserted -- verbatim the extract() pattern of runs/G3_EVENT_GC_20260906/src/build_daily_inputs.py.

Outputs: out/es_daily.parquet, out/zb_daily.parquet, out/inputs_manifest.json,
         out/build_inputs_log.txt
...
def build_returns(panel, held):
    ...
    old_close_prev. Never differences two contracts. (Verbatim logic from
    runs/G3_EVENT_GC_20260906/src/build_daily_inputs.py, itself verbatim from the GC autopsy.)"""
...
def main():
    P("=" * 100)
    P("=== G3_MEREBAL_20260906  STEP 0: build ES + ZB causal daily (day store, certified roll)")
    P("=" * 100)
