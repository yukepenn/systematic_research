"""SMV2AG common helpers -- shared by sub_424/425 scripts.
...
argument already exposed by member_states) plus the SMV2AD/SMV2R common_exec.e10_exec
...
This is a byte-identical copy of runs/SMV2AD_VOLMULT_CEILING/src/common.py with only
RUN/OUT repointed to this run's own directory -- per spec instruction to reuse SMV2AD's
control-arm daily curve (out/e10_daily_dev_control_1200.csv) and cache
(out/cache_control_1200.npz) verbatim rather than re-simulating, and to reuse its
portfolio-blend code (DAYONLY_DUAL6040 60/40 convention) verbatim.
"""
...
RUN = os.path.join(ROOT, "runs", "SMV2AG_ADAPTIVE_CLAMP")
OUT = os.path.join(RUN, "out")
SMV2AD_OUT = os.path.join(ROOT, "runs", "SMV2AD_VOLMULT_CEILING", "out")
DEV_END = dt.date(2026, 5, 31)
SEED, BLOCK, NBOOT = 20260808, 5, 10000

INCUMBENT_VMS = list(sm.VMS)  # 6..30 step2, 13 members
CEILING_FLOOR_PTS = 300.0     # 1200t incumbent, the floor the adaptive ceiling can never go below
...
def build_pend(bars, sig, vms, smax_ticks=1200.0, smin_ticks=40.0):
    """Verbatim SMV2AD build_pend -- fixed-ceiling ensemble builder, used only
    to reproduce/sanity-check the 1200t control (which is loaded from SMV2AD's
    cache, not re-simulated) and as a reference for the adaptive-ceiling variant
    in adaptive.py."""
...
def e10_exec(bars, tgt, comm_side=sm.MNQ_COMM_SIDE, point_value=sm.MNQ_POINT_VALUE):
    ...
    (instrumented e10_sim, MNQ costs, session flatten). Returns (daily_df[sess,net,contracts],
    bar_pos, bar_pnl)."""
    n = len(bars)
...
def metric_row(label, daily, tgt):
    ...
    same convention as SMV2AD/SMV2R) + top-10-day sum."""
...
def build_portfolio_6040(bars, tgt_solar, label):
    ...
    runs/SMV2AD_VOLMULT_CEILING/src/common.py construction (DUAL_HTF transform on
    the raw E10 target -> MNQ E10 executor -> 0.6/0.4 equal-vol blend with the
    frozen, UNCHANGED B-MOM E2 next-open leg). Returns (daily_solar_dual, portfolio_series,
    SIG_scalar, Tpp)."""
