"""G3_FTQGATE_20260906 -- STEP 0: build the two daily inputs (ES, ZB), ledger G00076.

Both series are built from the per-contract NT8 day .ncd store EXACTLY the way the GC autopsy /
G3_EVENT_GC built theirs (same reader research/multi_market/src/ncd_day.py, same causal
volume-crossover roll research/multi_market/src/roll.py, same self-financing point return +
identity gate vs roll.economic_returns, same seal assert).  The spec allows "es_1m -> daily (or
the ES daily store)"; the DAY STORE is used for BOTH legs so that ES and ZB carry the identical
certified causal-roll construction (documented choice; the 1-min substrate is NOT needed).

DELEV01: daily per-contract math in POINTS (ret_points, self-financing, basis-safe). No %.
...
def build_returns(panel, held):
    """Self-financing point return (identical to roll.economic_returns) with OHLC context.
    Never differences two contracts. (Verbatim logic from
    runs/G3_EVENT_GC_20260906/src/build_daily_inputs.py -> GC autopsy.)"""
...
def main():
    P("=" * 100)
    P("=== G3_FTQGATE_20260906  STEP 0: ES + ZB causal daily from the day store (G00076)")
    P("=" * 100)
    ...
    manifest = dict(seal=str(SEAL.date()))
    for root in ("ES", "ZB"):
        P("")
        P(f"--- {root} DAILY (per-contract day .ncd -> causal roll), method identical to GC autopsy")
        ...
        P(f"    tick size        {float(d['tick_size'].iloc[0])}   point value ${N.PV[root]}")
        P(f"    WROTE            {pq}")
        manifest[root.lower()] = dict(
            rows=int(len(df)),
    ...
    P("    SOURCE DOCUMENTED: ES from the DAY STORE (spec's allowed alternative to es_1m), so both")
    P("    legs share the identical certified causal-roll construction.")
    P("=" * 100)
    _fh.close()
