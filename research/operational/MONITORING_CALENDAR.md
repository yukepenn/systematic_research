...
| **🔴 ROLL WINDOW OPENS ~2026-09-02 — EARLIER THAN THE 09-10 PREVIOUSLY RECORDED HERE** (Sept expiry 09-18) | ⚠️ **CONTRACT ROLL — redeploy both paper legs on NQ 12-26.** VERIFIED from NT8 docs: *"NinjaScript strategies are not rolled forward and must be manually rolled over."* Nothing auto-rolls. ⛔ **ALL FOUR instruments must roll together.** ⭐ **DEADLINE TIGHTENED 2026-08-30:** the hardened classes carry a fail-safe that **refuses NEW ENTRIES from `NT8's stored rollover date − RollLeadDays(8)`** (`WeeklyEdgeP1PCT_v2.cs:460`). NQ's stored roll is ~09-10, so **the book likely stops taking entries ~09-02** — i.e. the real deadline is the week of Sept 1, not Sept 10. **EXITS ARE NOT GATED** (a blocked book still manages open positions), so this fails safe, not dangerous. Procedure in `NT8_RUNBOOK.md`. | `WeeklyEdgeP1PCT_v2` + `WeeklyEdgeXMConflict_v3` on DEMO8383477 | `PAPER_DEPLOYMENT_20260830.md` | XM's `EsInstrument`/`RtyInstrument`/`YmInstrument` must be set to **ES/RTY/YM 12-26** at redeploy (they default to 09-26). ✅ The root-only guard bug is **FIXED in `_v3`** (it now checks the contract month), so a partial roll is caught rather than trading Dec NQ against Sept secondaries silently — but roll all four anyway. Flatten before swapping; record the swap in the deployment file. **The exact block date resolves on the first realtime bar and is logged as `ROLL-PLAN blockNewEntriesFrom=…` — read it from the NT8 log and pin this row.** |
