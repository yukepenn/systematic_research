...
| **🔴 ~2026-09-10 (HARD DEADLINE; Sept expiry 09-18)** | ⚠️ **CONTRACT ROLL — redeploy both paper legs on NQ 12-26.** VERIFIED from NT8 docs: *"NinjaScript strategies are not rolled forward and must be manually rolled over."* Nothing auto-rolls. ⛔ **ALL FOUR instruments must roll together** — XM's guard compares only the root (`"ESZ6".StartsWith("ES")` is true), so a partial roll trades Dec NQ against Sept secondaries with the mismatch flag still false. Procedure in `NT8_RUNBOOK.md`** | `WeeklyEdgeP1PCT_v1` + `WeeklyEdgeXMConflict_v2` on DEMO8383477 | `PAPER_DEPLOYMENT_20260830.md` | XM's `EsInstrument`/`RtyInstrument`/`YmInstrument` must be set to **ES/RTY/YM 12-26** at redeploy (they default to 09-26). Flatten before swapping; record the swap in the deployment file. |
