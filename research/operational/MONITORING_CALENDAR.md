...
| **🔴 ROLL DEADLINE — XM STOPS ENTERING 2026-09-06, P1 2026-09-08 (RESOLVED FROM NT8, NOT ESTIMATED)** | ⚠️ **CONTRACT ROLL — redeploy both paper legs on NQ 12-26.** VERIFIED from NT8 docs: *"NinjaScript strategies are not rolled forward and must be manually rolled over."* Nothing auto-rolls. ⛔ **ALL FOUR instruments must roll together.** ⭐ **RESOLVED FROM NT8 AT THE 2026-08-30 18:00:00 ET SESSION OPEN** — no longer an estimate. The hardened classes refuse **NEW ENTRIES** from `stored rollover − RollLeadDays(8)` (`WeeklyEdgeP1PCT_v2.cs:460`), and the first realtime bar logged the resolved plan:<br>· `P1  ROLL-PLAN blockNewEntriesFrom=2026-09-08  earliestStoredRollover=2026-09-16 [NQU6:09-16]`<br>· `XM  ROLL-PLAN blockNewEntriesFrom=2026-09-06  earliestStoredRollover=2026-09-14 [NQU6:09-16 ESU6:09-14 RTYU6:09-15 YMU6:09-18]`<br>⇒ **XM binds FIRST, on 09-06, because ES rolls earliest (09-14)** — the multi-series leg takes the MIN across all four. **Roll both legs by Fri 2026-09-04.** (An earlier row here estimated ~09-02 from a guessed 09-10 rollover; the machine says 09-06/09-08 — estimate superseded by measurement.) **EXITS ARE NOT GATED**, so a missed roll stops new risk rather than stranding a position. Procedure in `NT8_RUNBOOK.md`. | `WeeklyEdgeP1PCT_v2` + `WeeklyEdgeXMConflict_v3` on DEMO8383477 | `PAPER_DEPLOYMENT_20260830.md` | XM's `EsInstrument`/`RtyInstrument`/`YmInstrument` must be set to **ES/RTY/YM 12-26** at redeploy (they default to 09-26). ✅ The root-only guard bug is **FIXED in `_v3`** (it now checks the contract month), so a partial roll is caught rather than trading Dec NQ against Sept secondaries silently — but roll all four anyway. Flatten before swapping; record the swap in the deployment file. **The exact block date resolves on the first realtime bar and is logged as `ROLL-PLAN blockNewEntriesFrom=…` — read it from the NT8 log and pin this row.** |
