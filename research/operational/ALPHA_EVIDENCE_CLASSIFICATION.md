...
| **`XM_CONFLICT_v2`** | **PARITY-CERTIFIED** · 🔴 **LIVE-ENABLED 2026-09-01 → WITHDRAWN TO OBSERVATION 2026-09-05** (owner decision, `OWNER_DECISION_20260905_XM_WITHDRAWN.md` — PRECAUTIONARY, not an evidence-based demotion: its live record is n=0 completed trades). **Certification NOT revoked; the evidence class below is unchanged.** ⚠️ `SHADOW-VALIDATED` was SKIPPED while it was live, exactly as for P1 | **REGIME-LOCAL by data availability** | ES/RTY/YM substrates **begin 2022-01-02**, so no 2006–2021 test exists *or can be built*. Strong modern evidence + coherent mechanism ≠ structural evidence. N = 348, discovery-consumed |
| `Portfolio B` (P1+XM inverse-vol) | **RESEARCH ONLY** | **REGIME-LOCAL** *(inherited)* | contains XM and is measured only on the modern common window — **rule 2**. ~~Also: no integer-contract mapping exists (`OQ-6`)~~ — ✅ **corrected 2026-09-01: `M_11` WAS selected and owner-ratified 2026-08-30, and OQ-6 is CLOSED.** M_11 is **not** an implementation of B (B is inverse-vol); **B's own** mapping remains unselected |
...
| **`ESNQ_V1`** (cross-market ES↔NQ) | ⛔ **CLOSED — GATES FAILED** | **DEVELOPMENT-ONLY, DISCOVERY-CONSUMED** | X1/X5/X6/X7 fail: net **−$18,113.79**, **−$503.16/session**, **OOF corr +0.0034**, cost/|gross| **55.5 %**, 0 of 4 quartiles positive. NQ-only control also negative. All structural gates passed first. ⚠️ **Closed at exact scope only** — price-side, 60 s, these 11 features, Ridge, 200 ms embargo, 44 dev sessions. **BLIND EFFECTIVE_14 UNSPENT AND UNREAD** · `runs/ESNQ_V1_20260828/` |
| **`BBO_BLIND_POOL` (19 sessions)** | **FROZEN ASSET** | **BLIND HISTORICAL — FALSIFIER-GRADE** | 🔴 **CONTENT-EXPOSURE RECORDED 2026-09-06 (verified): `2026-05-05` had its minute-close BID/ASK materialized by `runs/NQ1M_BIDASK_EXTRACT_20260906/` and published as SPREAD-only statistics — a pool member's QUOTE content was read.** ⚠️ **Assessment: SPREAD was exposed, NOT the RETURN/outcome the falsifier tests** (same logic as the `2025-08-13` metadata case below — the outcome-falsifier status of the RETURNS test is preserved), **but any future SPREAD/cost-conditioned falsifier on this pool is compromised for `2026-05-05` specifically.** Do NOT extend; owner-flagged (`OWNER_QUEUE.md`). The extraction was a $0 burned-window cost measurement that did not know the date was pooled (pools are frozen by NAME, and a spread reader cannot see the freeze). **Now: 17 pristine-never-materialized · 1 (`2025-08-13`) METADATA-EXPOSED · 1 (`2026-05-05`) SPREAD-CONTENT-EXPOSED.** — **19 outcome-unconsumed RTH-complete pre-seal NQ BBO sessions — of which 1 (`2025-08-13`) METADATA-EXPOSED** (its NQ CSV was transiently written and deleted unread in the exporter incident; no outcome status changes because metadata was exposed), **ELIGIBILITY RULE FROZEN AT `022c543`** (tree contains only `SPEC.md`), **REALIZED MANIFEST FIRST MATERIALIZED AND HASH-FROZEN AT `17bbb2d`**; normalized-content sha256 `92010fc6…2b8e` (the as-recorded `84a8575a…` was the CRLF working-tree hash — same content). **Returns NOT read**. The 99-vs-123 gap was **entirely a definition difference** (97 both · 19 RTH-only · 2 full-session-only) and the old "no pool exists" verdict was **correct under its own 23-hour criterion**. ⚠️ **MDE $2,996/session at n=19 — it can FALSIFY a large claim at 5.7 σ and cannot CONFIRM a modest one.** Governance: a genuinely different mechanism, frozen without reading it, one shot · `runs/BBO_COMPLETENESS_RECENSUS_V1_20260828/` |
| **ES BBO price-side** | **PARTLY CONSUMED** | **20 sessions genuinely unread** | ⚠️ **RE-CENSUSED 2026-08-28 after ESNQ. The earlier "64 unread" is stale twice over.** The asset is **79** RTH-complete pre-seal sessions, not 64 — the earlier count used the stricter as-run window. Exact split, from manifests not arithmetic: **44 outcome-consumed by ESNQ development · 15 in the ORIGINAL blind manifest (14 effective) · 20 genuinely unread and outside any blind set** — 44+15+20 = 79, exact. *(`64 − 44 = 20` would have reached the right number by two offsetting errors.)* ⚠️ **Low prior**: NQ price-side 60 s gave corr 0.0072 and ESNQ cross-market 60 s gave corr 0.0034 · `runs/ESNQ_V1_20260828/out/es_recensus.json` |
| **`VOLUME_LIQUIDITY_V1`** (participation / liquidity premium) | ⛔ **CLOSED — 10 of 12 GATES FAILED** | **DEVELOPMENT-ONLY, DISCOVERY-CONSUMED** | **Gross −$17,033.50 BEFORE any cost**; net **−$54,330.30**, Sharpe **−0.486**, drawdown **438 of 458 weeks**, **all 21** leave-one-root-out negative, and the object sits at the **56.5th** percentile of an exhaustive 363-shift temporal null and the **39.8th** of a 500-replicate within-sector identity placebo — **it is its own null**. The **mirror also loses** (−$20,263.30): same |positions|, same $37,296.80 turnover, so **there is nothing to invert**. Genuinely market-neutral (beta −0.0093, R² 0.0115) and **worse than simply being long everything** (+$23,924.86). ⚠️ **Closed at exact scope, and the negative is BOUNDED not absolute**: week-clustered SE 0.0101 means any weekly cross-sectional effect below **|r| ≈ 0.02** is invisible at n = 444 weeks · `runs/VOLUME_LIQUIDITY_V1_20260828/` |
| **Volume DATA capability** | **DATA-CAPABLE** | **measured, no alpha** | `VOLUME00`: **21 roots, all 6 sectors, 17 years, nothing excluded.** Contract-specificity settled by a **known-merged positive control** — V2/V3 read **0.0101 %** and **0.0011** on `db/day` but **98.86 %** and **1.0000** on the merged payload, so the test **discriminates**. ⚠️ **`ROOT_TOTAL` adopted**: the same-day contract-switch log-volume jump exceeds 1 MAD-unit on **87.2 %** of rolls, so designated-contract volume is mechanically discontinuous and a forced roll would have manufactured a long signal out of the calendar. Cross-source check **`REFERENCE UNAVAILABLE`** (the `db/minute` payload is delta-compressed; `GetBars` is disqualified as the **writer** of the store under test) · `runs/VOLUME00_20260828/` || **`P1/ABS`** | **PROSPECTIVE CHALLENGER** | **BURNED / DISCOVERY-CONSUMED** | PCT ahead in every fixed window and the mechanism is isolated (`ABS_LOOSE` control, p 0.940), but the paired **magnitude** test is p 0.058. Direction overwhelming, dollars not established. Forward data decides · `runs/ABS_PCT_ADJUDICATION_20260827/` |
| **PROGRAM B · intraday opportunity density** | ⛔ **BOTTLENECK MEASURED — NO FREE EXPANSION LANE** | **DESCRIPTIVE / DISCOVERY-CONSUMED** | The incumbent does **not** under-re-enter (**3.340 trades/active session**, 5th–8th entries its **best** bucket at $180.88/trade) and does **not** ignore the overnight session (**23 of 24 hours**, **63.3 % of entries outside RTH**). Its one real hole is that it is **completely flat on 420 of 1,058 sessions (39.7 %)** — and those sessions carry **84 %** of active-session range and **100 %** contain ≥3 ten-point reversals. `A-C1` passed at **77.9 %**; **`A-C2` FAILED** — the only two state families there are the **entry threshold** (forbidden mining) and the **mirrored short leg** (falsified 5×, `DEAD/FALSIFIED`). Arming identity `K·g·(1+dL) ≥ 16` verified against frozen `votes()`, **0 disagreements / 1,620,044 bars** · `runs/NQ_OPPORTUNITY00_20260828/` |
| **Reference NQ trader (campaign #6 object)** | **RETIRED AS A BENCHMARK** | **SCREENSHOT-ONLY, NOT A LEDGER** | 164 images; **no source, no statement, no ledger, no per-trade timestamp**. `8.26 trades/day` is one cell of one **BACKTEST** at **zero slippage**, one sleeve of several run concurrently. Per **in-market hour** he earns **$42.79** (**$33.47** re-costed on our model) against `P1/PCT`'s **$96.18**; at a common fixed DD his edge falls **1.40× → 1.09×**. **`P1/PCT` is NOT descended from him** — shared indicator ancestor only, and `ORIGINAL_PARITY` was never tested. Whether **his** later same-day trades paid is **UNKNOWABLE** from the fixed corpus · `research/intraday_opportunity/` |
