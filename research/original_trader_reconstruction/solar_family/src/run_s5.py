"""OTR_S5_REENTRY_QUALITY: T3 quality gates (strong-only / reclaim-TV)."""
import csv
import json
import os
import sys
...
OUT = os.path.join(ROOT, "runs", "OTR_S5_REENTRY_QUALITY", "out")
...
WINDOWS = {"W_0400_1600": win(240, 960), "EXCL_0200_0800": win(480, 120)}
SHAPES = {
    "E13_R1": dict(entry_types=(1, 3), reverse_on_flip=True),
    "E13_P0": dict(entry_types=(1, 3)),
}
QUALITY = {"Q1_strong": dict(t3_strong_only=True), "Q2_reclaim": dict(t3_reclaim_tv=True)}


def main():
    print("[S5] loading ledger ...", flush=True)
    bars = load_ledger(LEDGER)
    results = []
    for sname, skw in SHAPES.items():
        for wname, wfn in WINDOWS.items():
            for qname, qkw in QUALITY.items():
                cid = f"{sname}|{wname}|FF|{qname}"
                pol = WrapperPolicy(name=cid, comm_side=0.0, entry_time_mask=wfn,
                                    flat_time_mask=(lambda m, f=wfn: ~f(m)), **skw, **qkw)
                fp = run_wrapper(bars, pol)["fingerprint"]
                D, errs = distance(fp)
                results.append({"cell": cid, "D": D, "fingerprint": fp, "errors": errs})
                print(f"[S5] {cid:38s} D={D:7.3f} trades={fp.get('trades',0):5d} WR={fp.get('win_rate_pct',0):6.2f} "
    ...
    print("[S5] best:", results[0]["cell"], "D=", results[0]["D"], flush=True)
