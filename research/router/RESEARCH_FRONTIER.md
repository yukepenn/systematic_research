...
| **2** | ⭐ **FREE UNACQUIRED DATA — the free tier is NOT exhausted** | ⚠️ **THIS ROW EXISTS BECAUSE THE PREVIOUS VERSION OF IT WAS WRONG — AND GENESIS WAVE 1 THEN CORRECTED THIS VERSION TOO** (`runs/GENESIS_W1_FORENSICS_20260828`): ~~VX/VXM already in NT8~~ **overstated** (probe residue; VXM zero data) — the VIX complex is free via **Cboe CDN**, and Wave 1 added **`VXN`, the NQ-native vol index, zero prior repo mentions** (+ VVIX/VIX9D/SKEW/OVX/GVZ); ~~$TICK to ~2013~~ **downgraded** to two point probes, continuity unestablished, gated behind a structural seal guard; **MNQ tick 187/128 verified exactly** (`build_registry.py:198`, and the registry never scans `db/tick` — ES tick 126 dates full BBO equally invisible); ~~nine~~ **six-plus-one** genuinely unextracted 1-min stores (ES/RTY/YM were already extracted AND consumed); free Cboe `VIX_History` 1990→, CFE **OI** 2004→; **CFTC COT**, unprobed · authoritative map now **`research/genesis/GENESIS_DATA_ATLAS.md`** |
...
> reachable** — ⚠️ *as corrected by GENESIS Wave 1:* the VIX complex **via the free Cboe CDN
> including the NQ-native `VXN`** (not "sitting in NT8" — that was probe residue), deep `$TICK`
> as an unproven-continuity lead, and an entire MNQ tick store hidden by a hard-coded symbol
