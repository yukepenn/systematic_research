...
| **1** | **ES ↔ NQ SUB-MINUTE CROSS-MARKET** | ✅ **the highest-EVI runnable question, and the first microstructure lane that can be BLIND-CONFIRMED.** Genuinely different information — **cross-market transfer**, not single-instrument quote dynamics, so the corrected NQ-only null does **not** bear on it. W122's NULL was a **1-minute** family. **59 overlapping RTH-complete sessions**; **15 have BOTH sides unread**. Execution is the already-measured NQ bid/ask crossing. ⚠️ **EXPORT-GATED (~3.7 GB)** and its confirmation set is **falsification-grade, not validation-grade** · `runs/ESNQ_V1_20260828/SPEC.md` |
