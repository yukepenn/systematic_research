...
| — | ~~**ES ↔ NQ SUB-MINUTE CROSS-MARKET**~~ | ❌ **DEVELOPMENT FAILED X1/X5/X6/X7 → `ESNQ_V1` CLOSED.** Net **−$18,113.79**, **−$503.16/session**, 30.6 % positive sessions, **OOF corr +0.0034**, cost/|gross| **55.5 %**. The NQ-only control is **also** negative (−$55.79/session), so neither information set works at 60 s here. Structural gates all passed first (P0-1 two-sided causality, P0-2 **0 violations on all 14,564 decisions**, P0-3 parity — which caught a real streaming defect pre-result). **BLIND POOL UNSPENT**: `mu_claim = $0.00` → power 0.000 → WITHHELD · `runs/ESNQ_V1_20260828/` |
