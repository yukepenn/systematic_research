...
`ESNQ00`.** Any earlier "as of" wording
...
| — | ~~**Multi-market TSMOM V1**~~ | ❌ **DEVELOPMENT GATES FAILED 3 of 6** — Sharpe **0.226** (<0.30), **5 of 9** positive years (<6), **72.3 %** equity-index concentration (>60 %) · net $10,167 but **cost drag 47.2 % of gross**, scale-invariant · net earned entirely 2010–2014; last four years **−$10,003** · long +$10,821 vs short +$923, i.e. **long-equity beta — the least diversifying thing a book anchored on NQ could find** · **VALIDATION AND FINAL HOLDOUT NOT READ** · `research/multi_market/TSMOM_V1_DEVELOPMENT.md` |
| — | ~~**TSMOM V2 (252d slow trend)**~~ | ❌ **FAILED VALIDATION on V2-G3** — 2 of 4 positive years. Net $15,080 at **Sharpe 0.577**, cost drag 15.4 %, top sector 47.1 % — **5 of 6 gates passed** — but 2020 (+$12,068) and 2022 (+$6,111) carry everything while both calm years lose. A **crisis-regime payoff, not a steady premium.** **FINAL HOLDOUT UNSPENT** · `runs/TSMOM_V2_SLOWTREND_20260827/` |
...
| — | ~~**`MS-BBO-CANDIDATE-1`**~~ | ⛔ **VOID 2026-08-28 — IT READ THE FUTURE.** `np.arange(-30,0) * NS` overflows **int32** on Windows/NumPy 1.26, so 15 of 30 path offsets were **positive**, reaching **+2.065 s past the decision instant**. The leak was **$6,910.64/session = 134.8 %** of the reported result; the causal object earns **−$1,785.88/session at OOF corr 0.0072**. Found by an **independent streaming re-implementation**, not by any null, placebo or mirror — those all inherit a feature-construction leak · `runs/MSBBO_DEPLOYMENT_FREEZE_20260828/` |
| — | ~~**CARRY_V1**~~ | ❌ **DEVELOPMENT FAILED C6/C7 → CLOSED.** 6 of 8 gates passed — net $71,413, Sharpe **0.719**, 7/9 positive years, cost drag **7.9 %**, two-sided causality probe clean — but **SI alone is 84.1 %** of positive root contribution and **metals 98.5 %** of positive sector contribution. A silver-vs-gold pair trade in a four-sector costume. **VALIDATION AND FINAL HOLDOUT NOT READ** · `runs/CARRY_V1_20260828/` |
| **1** | **A NEW INFORMATION SURFACE — the honest rank-1** | ⚠️ **The project has no candidate.** Every materialized surface it can currently reach is closed: action value, HTF, internals ×2, Last-only, BBO (void), TSMOM ×3, carry. **Rank 1 is therefore acquisition or re-census, not another model on the same substrate.** The two cheapest live leads are the ceiling re-census (row 2) and the ES/NQ export (row 3) |
| **2** | **BBO quote-completeness RE-CENSUS** | 🔍 **cheap, and it may reopen a lane.** The 2026-08-27 split recorded a quote-FULL ceiling of **99**, all consumed. Today's file-level census finds **123** NQ RTH-complete days, **116 pre-seal**. The criteria differ (file presence vs measured completeness) so **no blind pool is claimed** — but the gap is worth one afternoon, and it decides whether any future BBO-class object can ever be blind-confirmed · `runs/ESNQ00_CAPABILITY_20260828/` |
| **3** | **ES ↔ NQ sub-minute** | ⚠️ **DATA-CAPABLE (59 overlapping RTH-complete sessions, 52 unexported) but QUESTION-VOID and POWER-OPEN.** §52's *"incremental beyond the frozen NQ-only BBO set"* has lost its baseline. Re-posing it as a joint ES+NQ discovery question is a **new preregistration** with its own multiplicity, and it is **export-gated** (multi-GB, multi-hour) |
| **4** | **PROSPECTIVE SHADOW** | ⬇️ **DEMOTED from rank 1.** Its entire EVI rested on `MS-BBO-CANDIDATE-1` (~200 decisions/session, no other test available ever). The remaining roster — `P1/PCT`, `P1/ABS`, `XM` — accrues **~1.6 decisions/week**, so a paired read needs years. Protocol and `SHADOW_START` kept; **infrastructure deliberately not built for a void object** · `research/operational/PROSPECTIVE_SHADOW.md` |
| **5** | Portfolio-B weighting/selection optimism · incumbent adjudication v2 | ✅ **DONE and CLOSED** — kept only so the row is not re-opened. Weighting optimism **1.4 %**, selection optimism **13.9 %** · `runs/PORTFOLIO_B_RECONCILIATION_20260827/` · `runs/ABS_PCT_ADJUDICATION_20260827/` |
