...
|---|---|---|
| — | ~~**`MS-BBO-CANDIDATE-1`**~~ | ⛔ **VOID 2026-08-28 — IT READ THE FUTURE.** `np.arange(-30,0) * NS` overflows **int32**; 15 of 30 path offsets were positive, reaching **+2.065 s past the decision instant**. Leak **$6,910.64/session = 134.8 %** of the reported result; causal object **−$1,785.88/session at OOF corr 0.0072**. Found by an **independent streaming re-implementation**, which is the only thing that could have found it — nulls, placebos and mirrors all recompute the same leaky features · `runs/MSBBO_DEPLOYMENT_FREEZE_20260828/` |
| — | ~~**CARRY_V1**~~ | ❌ **FAILED C6/C7 → CLOSED.** 6 of 8 gates passed (net $71,413, Sharpe **0.719**, 7/9 positive years, cost drag 7.9 %, two-sided causality clean) but **SI alone is 84.1 %** of positive root contribution and **metals 98.5 %** of sector contribution. A silver-vs-gold pair in a four-sector costume. **VALIDATION AND FINAL HOLDOUT NOT READ** · `runs/CARRY_V1_20260828/` |
| — | ~~**more carry roots**~~ | ❌ **CLOSED-BY-DATA.** The obvious `CARRY_V1` rescue — fetch roots so every sector has ≥3 and the `n_sector = 2` ±1 rank degeneracy disappears — is foreclosed by measurement: of 13 extra `db/day` roots, **10 are micros of existing roots** (zero new information) and none of `10YR`/`2YR`/`ZS` has ≥10 contracts · `runs/ASSET_CENSUS_20260828/` |
| **1** | **ES ↔ NQ SUB-MINUTE CROSS-MARKET** | ✅ **the highest-EVI runnable question, and the first microstructure lane that can be BLIND-CONFIRMED.** Genuinely different information — **cross-market transfer**, not single-instrument quote dynamics, so the corrected NQ-only null does **not** bear on it. W122's NULL was a **1-minute** family. **59 overlapping RTH-complete sessions**. ✅ **DEV_44 SUBSTRATE BUILT** (44 NQ + 44 ES parquet); clock contract **CLOCK-COMPATIBLE-BUT-SEMANTICS-UNKNOWN** with a positive control that detects a known +250 ms shift; **`CROSS_MARKET_ES_EMBARGO = 200 ms`** frozen. Blind confirmation at **EFFECTIVE n = 14** after a pre-outcome operational quarantine. ⚠️ **NO ALPHA RESULT EXISTS** — runner and result pending; confirmation set is **falsification-grade, not validation-grade** · `runs/ESNQ_V1_20260828/SPEC.md` |
| **2** | **Multi-market VOLUME / liquidity information** | genuinely different surface on a substrate that is **already local and free** — contract volume is in every `.ncd` record and has only ever been the roll criterion. ⚠️ **Permanently discovery-grade**: its dates are outcome-consumed by TSMOM and CARRY, so **no blind window can ever exist** for it, and it inherits family-selection debt |
| **3** | **ES-only BBO standalone** | best *evidence structure* available — 64 fully unread sessions could give development **and** blind confirmation from one pristine asset. ⚠️ **But the lowest prior**: it is the same hypothesis class that just returned **corr 0.0072** on NQ, in the same market structure. **Spending a pristine asset to re-learn a null is the wrong trade** |
| **4** | **NQ Last-only blind pool (141)** | **not runnable** — it is an asset, not a question. Governance requires a **genuinely different mechanism frozen without reading the pool**, and **none is on the table**. Its quote classes are `NONE` 101 / `PARTIAL` 40, so **no bid/ask crossing can be priced there** |
| **5** | **PROSPECTIVE SHADOW** | ⬇️ **demoted.** Its entire EVI rested on the void candidate. Remaining roster (`P1/PCT`, `P1/ABS`, `XM`) accrues **~1.6 decisions/week**. Protocol and `SHADOW_START` kept; **infrastructure deliberately not built for a void object** |
| **6** | **Owner acquisition** — options/dealer gamma, more event types | highest remaining *ceilings* but **owner-gated spend**, and §89 binds: exhaust the free surfaces first. Rank 1 is free-surface work, so this waits |
