...
| ~~NT8 Strategy Analyzer parity for `WeeklyEdgeX9a_v1.cs` / `WeeklyEdgeBmom_v1.cs`~~ | ⚠️ **NOT GATED — this row was false and is removed from the gate list, 2026-08-27.** The real Strategy Analyzer runs through the MCP's `RunStrategyBacktest` (add-on v1.13.9, NT8 8.1.8.1), which takes a **class name, not source**, and remains allowed; both legs of the book were parity-certified through it on 2026-08-27. ⛔ **Compiling our own source through the MCP is BANNED as of 2026-09-01** — it is a *remote* server whose terms owe us no confidentiality (CLAUDE.md §1): no `CompileNinjaScript` / `WriteNinjaScriptFile` / `ReadNinjaScriptFile`. **Install with a local `cp` into `Documents/NinjaTrader 8/bin/Custom/Strategies/` and verify by resolving the class — CLAUDE.md §6.** The claim was **inherited, never probed**, and cost a full day. **Standing rule, CLAUDE.md §6: never assert an action is owner-only without re-probing the tool surface that day.** Parity for these two is simply *not done*, which is a queue position, not a gate |
