...
LEDGER = os.path.join(OUT, "we_xm_xm2.csv")


def main():
    t0 = _time.time()
    out = open(os.path.join(OUT, "parity_v2.txt"), "w", encoding="utf-8")
    ...
    N[["session_date", "nq_drive", "broad_composite", "conflict_flag", "desired_direction",
       "decision_ready"]].to_csv(os.path.join(OUT, "nt8_decisions_v2.csv"), index=False)
    ...
    if len(bad):
        ...
        bad[cols].to_csv(os.path.join(OUT, "decision_mismatches_v2.csv"), index=False)
        ...
        if len(bad) > 40:
            P_(f"        ... {len(bad)-40} more in decision_mismatches_v2.csv")
    ...
    P_("=" * 122)
    d = json.loads(open(os.path.join(OUT, "nt8_raw_xm_v2.json"), encoding="utf-8").read())
    rr = d["result"]
    ...
    pd.DataFrame([dict(gate=x[0], spec=x[1], observed=x[2], verdict="PASS" if x[3] else "FAIL")
                  for x in g]).to_csv(os.path.join(OUT, "gates_v2.csv"), index=False)
