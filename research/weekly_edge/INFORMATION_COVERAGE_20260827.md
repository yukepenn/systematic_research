...
| **market internals (TICK/ADD/TRIN)** | ✅ **ACQUIRED — lane MARGINAL** | `✗ no data` was a statement about this REPO, not the connection. `runs/INTERNALS_ACQUIRE_20260827/` has since **acquired `$TICK`, `$TRIN` and `$VIX` at 1-minute for 2022-01-03 → 2026-07-31 (1.34 M bars, ~1,147 sessions each, $0)**. They cover **764 of 2,139 P1 decisions = 35.7 %, **5.4× the order-flow lane** — internals exist for EVERY RTH session, so coverage is not the binding constraint. Gate: MDE **1.07×** the covered mean session-scoped, **1.31×** full-horizon — **MARGINAL, the closest any new surface has come**, versus 3.11× / 4.61× for order flow. ⚠️ **Permanent ceiling: P1 is 64 % overnight, so internals can never speak to two-thirds of the book.** `$ADD` and `$VXN` genuinely absent on failed probes. **Information test NOT run — it needs its own preregistration** |
