...
| **BBO / micro-price / trade imbalance** | ⚠️ **REOPENED — NOT BLOCKED** | `DATAGATE_ORDERFLOW_20260827` measured the **substrate** (48 sessions ⇒ 71 of 2,131 P1 entries, 3.3 %, MDE $564/entry). `runs/DATA_CAPABILITY_AUDIT_20260827/` measured the **disk**: the local NT8 tick store holds **310 NQ sessions with `Last`** and **168 BBO-complete** (Last+Bid+Ask), 2025-08-10 → 2026-07-31, of which **262 / 123 have never been extracted**. **Two lanes, do not merge them:** signed flow (tick rule, `Last` only) **reaches the ~300-session target at 310**; quote-based features reach only **168 — 3.7× better, still short**. The bottleneck was **manual labour**, not entitlement or money |
...
| surface | why |
|---|---|
| ~~BBO / order flow / micro-price~~ | ⚠️ **NO LONGER UNAVAILABLE** — 262 `Last` and 123 BBO-complete NQ sessions sit unextracted on local disk. See the row above |
