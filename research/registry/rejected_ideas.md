...
- **Naive NQ-block capital scaling (C01T2_KSCALE)**: trunc-block NQ/MNQ mixing at K=2/3 DESTROYS Sharpe (−0.19) — boundary churn exceeds the maximum commission saving; NQ slippage per unit equals MNQ's so only commission is recoverable. All-MNQ linear scaling is the correct K>1 implementation. A hysteresis-block variant needs a fresh preregistration and has a hard ~$22k/NQ-eq ceiling at K=2.
