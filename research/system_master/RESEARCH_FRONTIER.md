...
| R2 (entry timing/pullback) | Conditional entry confirmation (persistence, pullback-then-reclaim) improves entry quality without suppressing rare right-tail entries | **VALIDATING** | confirm_bars=2 entry-confirmation overlay | `runs/R2_ENTRY_TIMING/REPORT.md` | Every primary metric improves on both NQ/MNQ simultaneously (net +8.6%/+9.4%, Sharpe +11.2%/+12.0%, CDaR95 better); tail-dollar-positive (+$32,850, opposite of R1); survives 2-tick stress; doesn't miss 2025-04-09; independently adversarially audited, no bug found | Early NT8/CrossTrade validation (binding, not yet run), formal LOYO, proper child spec |
