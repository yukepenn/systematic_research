...
| **SolarWaveOneContractMNQ_Final** (Product B, BEST_ONE_MNQ) | ✅ registered | ✅ clean | ⚠️ **GENUINELY BACKTESTED, PARITY PENDING** (99.42% trade-exact, but daily corr 0.8996 / net Δ 0.78% miss the bar) | first-ever real MNQ Strategy Analyzer run for this policy; gap diagnosed as NQ-vs-MNQ price-basis divergence in the CURRENT Python reference (which fills MNQ at NQ-scaled prices, not genuine MNQU6 prints) — a known residual class (see EVIDENCE_MAP_RAW.md's E10Master precedent), not a defect in the Final file. Fix requires a genuine-MNQ-price reference rebuild (`GetBars` attempt this run returned empty; unresolved). See `runs/PRODUCTB_ONECONTRACT_FINAL/REPORT.md`. |
