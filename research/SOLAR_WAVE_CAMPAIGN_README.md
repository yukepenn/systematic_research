...
> **Moved from the repo root to here on 2026-08-09** — the root `README.md` is now the
> whole-repo landing page, not this campaign's. Two further campaigns have run since and are
> now the active work: **[`/BASELINE_MODELS.md`](../BASELINE_MODELS.md)** (the 3 current
> canonical objects) and **`research/system_master/CURRENT_TRUTH.md`** (SYSTEM_MASTER —
> portfolio + one-contract construction, active, updated continuously), plus
> **`research/scalping_lab/CAMPAIGN_STATE.md`** (short-horizon scalp search, phase complete).
> For overall repo orientation start at **[`/README.md`](../README.md)** or
> **[`/MAP.md`](../MAP.md)**. This file is kept exactly as the original campaign left it — its
> own content below is accurate for what it covers, just not the whole repo anymore.
