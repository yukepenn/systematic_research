...
// SolarWaveE10Master_v1 — the R5-E10 executable champion as ONE NinjaScript strategy.
//
...
{
    public class SolarWaveE10Master_v1 : Strategy
    {
        ...
        private void OpenBarLog()
        {
            try
            {
                ...
                barLog.WriteLine(string.Format(CultureInfo.InvariantCulture,
                    "# SolarWaveE10Master_v1 SM={0} SS={1} WWS={2} VP={3} smin={4} smax={5} exec={6} instrument={7} period={8}",
            ...
            {
                Print("SolarWaveE10Master_v1 bar log open failed: " + ex.Message);
                barLog = null;
        ...
        private void Flush()
        {
                ...
                if (!string.IsNullOrEmpty(ExportDir) && fillLog != null)
                {
                    ...
                    using (StreamWriter w = new StreamWriter(Path.Combine(ExportDir, Tag + "_fills.csv"), false))
                    {
                        w.WriteLine(string.Format(CultureInfo.InvariantCulture,
                            "# SolarWaveE10Master_v1 execs={0} exec_instrument={1}", execCount, ExecInstrument));
            ...
            {
                Print("SolarWaveE10Master_v1 export failed: " + ex.Message);
            }
        }
