// SolarWaveOneContractNQ_v6_R2CONFIRM — CHALLENGER candidate, NOT the incumbent.
//
// Base object: SolarWaveOneContractNQ_v5 (current BEST_ONE_NQ incumbent, unchanged, still the
// control -- see /BASELINE_MODELS.md). This file is the NinjaScript twin of the Python-side
// R2_ENTRY_TIMING "confirm=2" candidate (runs/R2_ENTRY_TIMING/REPORT.md, src/construct.py),
// built ONLY for early executable validation (directive standing rule sec28-29: validate a
// genuinely-promotable candidate against real NT8 BEFORE spending further robustness budget).
// It is a RESEARCH CHALLENGER: not deployed to any live-adjacent state, not parity-certified,
// not renamed to a "_Final"/incumbent-superseding name, and the v5 incumbent file is untouched.
//
// WHAT CHANGED FROM v5, AND ONLY THIS:
//   A NEW commitment from flat (p==0, M crosses +-EntryLevel) is no longer taken immediately.
//   It is ARMED, and only actually submitted once M has stayed beyond +-EntryLevel on the SAME
//   side for ConfirmBars (=2, frozen from the preregistered Python grid, seq R2) ADDITIONAL
//   bars; if M reverts at any point during confirmation, the candidate entry is cancelled and
//   flat evaluation resumes fresh next bar. Exits and reversals from an EXISTING position are
//   completely unchanged -- this is a conditional entry filter, not a universal delay (directive
//   sec12's explicit warning re: S2_SELTIME's blanket-exclusion failure mode).
// WHY: Python diagnostic (runs/P0_TRADESTATE_AUTOPSY, runs/R2_ENTRY_TIMING) found 77/1978 (3.9%)
// incumbent entries where M reverts within 1 bar of crossing EntryLevel cost -$1,160.75 avg vs
// +$213.28 avg for persistent entries, with ZERO of the top-20 incumbent winners affected.
// Python confirm=2 candidate improves net/Sharpe/CDaR95 simultaneously on BOTH NQ ($328,037.52,
// Sharpe 1.238) and MNQ ($31,268.60, Sharpe 1.180) economics, is tail-dollar-positive
// (+$32,850 across top10/bottom10 buckets), survives 2-tick cost stress, does not miss
// 2025-04-09, and was independently adversarially audited (fresh agent, no bug found).
//
// ORIGINAL v5 HEADER BELOW (unchanged, historical context for the base object this challenger
// is built from):
//
...
// BUILD TAG: SolarWaveOneContractNQ_v6_R2CONFIRM 2026-08-09 (R2 entry-confirmation challenger).
...
{
    public class SolarWaveOneContractNQ_v6_R2CONFIRM : Strategy
    {
        private const string BuildTag = "SolarWaveOneContractNQ_v6_R2CONFIRM 2026-08-09";
        ...
        // R2 entry-confirmation state (the ONLY behavioral addition vs v5). A new commitment
        // from flat is armed on the side it first crosses EntryLevel, and only submitted once
        // that side has held for ConfirmBars ADDITIONAL bars in a row.
        private int armedSide;
        private int armedBars;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "SolarWaveOneContractNQ_v6_R2CONFIRM";
                ...
                ConfirmBars = 2; // frozen from the preregistered R2 Python grid, not a free parameter
                ExportDir = ""; Tag = "nq_v6_r2confirm2";
            }
            ...
            else if (State == State.DataLoaded)
            {
                ...
                Print(string.Format(
                    ...
                    EntryBlockMinutesBeforeClose, ForcedFlatMinutesBeforeClose, ConfirmBars));
            }
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            else if (p == 0)
            {
                ...
                {
                    int raw = 0;
                    if (M >= EntryLevel) raw = 1;
                    else if (M <= -EntryLevel) raw = -1;

                    if (raw == 0)
                    {
                        armedSide = 0; armedBars = 0;
                    }
                    else if (armedSide == raw)
                    {
                        armedBars++;
                        if (armedBars >= ConfirmBars)
                        {
                            tgt = raw;
                            armedSide = 0; armedBars = 0;
                        }
        ...
        [NinjaScriptProperty] [Display(Name="Confirm bars (R2 challenger)", Order=17, GroupName="OneLot")]
