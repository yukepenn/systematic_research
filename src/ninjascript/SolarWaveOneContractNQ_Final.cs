// SolarWaveOneContractNQ_Final — Product B final deliverable, BEST_ONE_NQ.
//
// This is a behavior-preserving packaging of SolarWaveSMOneLot_v1.cs (SM14, seq 318
// adopted) for the single-instrument NQ final artifact required by
...
// SolarWaveSMOneLot_v1.cs. The only additions are (1) an execution-instrument guard so
// this file refuses to trade if attached to anything but an NQ chart, (2) effective-
// parameter logging (LIVE_READINESS_CHECKLIST.md item 15), (3) hardcoded EntryLevel/
// ExitLevel defaults to the frozen seq-318 values (still exposed as properties for
// Strategy Analyzer visibility, not to re-open the parameter search).
//
...
// bars, CME US Index Futures ETH template. SignalInstrument (NQ continuous, back-adjusted
// merge) drives all signal math regardless of execution instrument.
...
//   1 NQ: net $303,850.92, Sharpe 1.1197, maxDD -$58,517.44
// Strategy Analyzer parity (research file name, pre-refactor): 99.5% trade-exact, daily
// corr 0.9990, net delta 0.13% -- see research/system_master/NINJATRADER_PARITY.md. This
// Final file re-runs parity under its own class name (runs/PRODUCTB_ONECONTRACT_FINAL/).
...
// BUILD TAG: SolarWaveOneContractNQ_Final v1.0 2026-08-08 (Product B deliverable).
...
{
    public class SolarWaveOneContractNQ_Final : Strategy
    {
        private const string BuildTag = "SolarWaveOneContractNQ_Final v1.0 2026-08-08";
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "SolarWaveOneContractNQ_Final";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                // Execution-instrument guard: this Final file is BEST_ONE_NQ specifically.
        ...
        private void SubmitTarget(int tgt)
        {
            ...
            if (State == State.Realtime)
                return;
            // Execution-instrument guard: refuse to trade if not attached to NQ.
            if (instrumentMismatch)
                return;
