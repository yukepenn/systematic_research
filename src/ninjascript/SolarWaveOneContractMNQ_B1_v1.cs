// SolarWaveOneContractMNQ_B1_v1 — frozen SIMPLE01 "B1" rung, executable research object.
//
// Per research/system_master/SIMPLE01_MINIMUM_SYSTEM/out/00_SPEC_candidate_manifest.md sec1.2
// (SPEC-agent-frozen, no PnL viewed while writing it): B1 = "Solar + B-MOM, no HTF" — `mm`
// (the tiltState-gated TiltMult up-weight) is forced to the constant 1.0; `bmomPos` stays REAL;
// WSolar/WBmom/EntryLevel/ExitLevel/TiltRescale/the hysteresis state machine/the ops-layer
// session-close and entry-block rules are all UNCHANGED from the incumbent, as is the
// cross-series arrangement (signal=NQ primary, execution=MNQ added series). This is the ONE-LINE
// diff below and nothing else — no other decision-logic line in this file differs from
// SolarWaveOneContractMNQ_v5.cs. Per master directive sec17-25: this candidate is FROZEN, not
// retuned. This object is an IMPLEMENTATION-PARITY twin only — it reproduces SIMPLE01's Python
// B1 candidate exactly, it does NOT re-adjudicate B1's historical verdict. Any future promotion
// decision requires NEW evidence (locked-forward or protected pool), never further analysis of
// this already-consumed history alone (sec25). Uses ONE shared NQ-derived decision core with the
// NQ sibling (SolarWaveOneContractNQ_B1_v1.cs) — no independent MNQ alpha (sec19).
//
// ==========================================================================================
// ORIGINAL SolarWaveOneContractMNQ_v5 HEADER (kept verbatim below for provenance/diff traceability)
// ==========================================================================================
//
// SolarWaveOneContractMNQ_v5 — Product B BEST_ONE_MNQ, C4-compliance rebuild.
//
...
// BUILD TAG: SolarWaveOneContractMNQ_v5 2026-08-09 (W17 C4-compliance rebuild).
...
{
    public class SolarWaveOneContractMNQ_B1_v1 : Strategy
    {
        private const string BuildTag = "SolarWaveOneContractMNQ_B1_v1 2026-08-10";
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "SolarWaveOneContractMNQ_B1_v1";
        ...
        private void ExecWatchdog()
        {
            ...
            // NOTE (v3 -> v4 correction): the unindexed accessors Time/Close/CurrentBar are
            // BarsInProgress-RELATIVE in NinjaScript -- inside this BIP==1 handler `Time[0]`
            // is the EXECUTION series' own timestamp, not the decision series'. v3 read it as
            // the decision clock, so decTs always equalled execTs, the staleness gate always
            // returned, and the watchdog was silently inert (v3 reproduced v2 to the cent,
            // which is how this was caught). Index the series explicitly.
        ...
        protected override void OnBarUpdate()
        {
            ...
            double mm = 1.0; // B1 (frozen, SIMPLE01 sec1.2): HTF up-weight removed -- forced to 1.0, bmomPos stays real
